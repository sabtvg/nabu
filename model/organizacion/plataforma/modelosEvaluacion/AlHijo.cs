...
{
    public class AlHijo : ModeloEvaluacion
    {
        public AlHijo()
        {
            modeloDocumento = "AlHijo";
            nombre = "AlHijo";
        ...
        override protected string toHTMLContenido(Propuesta prop, Grupo g, string email, int width)
        {
            ...
            {
                ret += "<tr>";
                ret += "<td class='tema' style='vertical-align:top'><b>" + Tools.tr("alhijo.evaluacion.p" + q, g.idioma) + "</b>";
                if (Tools.tr("alhijo.evaluacion.tip" + q, g.idioma) != "")
                    ret += "<div class='smalltip' style='width:90%'>" + Tools.tr("alhijo.evaluacion.tip" + q, g.idioma) + "</div>";
                ret += "</td>";
